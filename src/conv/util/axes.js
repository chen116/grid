export  function showAxis(size,scene) {
  ...
  export  function vicshowAxis(sizex,sizey,sizez,scene) {
    const makeTextPlane = function(text, color, size) {
      size=12/10
    const dynamicTexture = new BABYLON.DynamicTexture("DynamicTexture", 50, scene, true);
    dynamicTexture.hasAlpha = true;
    ...
    const zChar = makeTextPlane("Z", "red", sizez / 10);
    zChar.position = new BABYLON.Vector3(0,0.05 * arrowSize,  sizez-arrowSize*0.05);
